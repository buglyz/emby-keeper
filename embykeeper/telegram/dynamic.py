...
def get_names(type: str, allow_ignore=False) -> List[str]:
    ...
    module_dir = Path(next(iter(typemodule.__path__)))
    for _, mn, _ in pkgutil.iter_modules(typemodule.__path__):
        if not allow_ignore:
            if mn.startswith("_") or mn.startswith("test"):
                continue
            module_file = module_dir / f"{mn}.py"
            if module_file.is_file():
                source = module_file.read_text(encoding="utf-8", errors="ignore")
                if re.search(r"__ignore__\s*=\s*True", source):
                    continue
            results.append(mn)
        else:
