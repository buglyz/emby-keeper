...
class BaseBotRegister(ABC):
    ...
    @property
    def retries(self):
        return self._retries if self._retries is not None else getattr(config.registrar, "retries", 1)

    @property
    def timeout(self):
        return self._timeout if self._timeout is not None else getattr(config.registrar, "timeout", 120)
