...
class Link:
    ...
    async def auth(self, service: str, log_func=None):
        ...
        async with authed_services_lock:
            ...
            # Remote auth is disabled by default so self-hosted deployments
            # keep working after the original auth service is retired.
            if os.getenv("EMBYKEEPER_ENABLE_REMOTE_AUTH", "").lower() not in {
                "1",
                "true",
                "yes",
                "on",
                ...
                self.log.debug(f"[gray50]skip remote auth for service: {service}[/]")
                return True

            # No cache, perform legacy remote auth
            if not log_func:
