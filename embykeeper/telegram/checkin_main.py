...
from embykeeper.schema import CheckinerConfig, TelegramAccount
...
class CheckinerManager:
    ...
    def __init__(self):
        ...
        self._scheduler_tasks: Dict[str, asyncio.Task] = {}  # phone/site -> scheduler task
        self._pool = AsyncTaskPool()

        self._account_change_handle = config.on_list_change("telegram.account", self._handle_account_change)
    ...
    def _start_scheduler(self, key: str, scheduler: Scheduler):
        ...
        task = self._pool.add(scheduler.schedule(), f"{key} 签到计划")
    ...
    async def _wait_pool(self):
        async for task in self._pool.as_completed():
            try:
                ...
                logger.debug(f"签到计划任务 {task.get_name()} 已取消.")
            except Exception as e:
                logger.warning(f"签到计划任务 {task.get_name()} 异常退出: {format_exception_summary(e)}")
    ...
    def _handle_config_change(self, *args):
        ...
        for phone in {key.split(".", 1)[0] for key in set(self._schedulers) | set(self._scheduler_tasks)}:
        ...
        for account in self._telegram_accounts():
            if account.enabled and account.checkiner:
                ...
                if scheduler:
                    self._start_scheduler(account.phone, scheduler)
    ...
    def stop_account(self, phone: str):
        ...
        # Cancel main account scheduler task
        ...
        keys_to_remove = []
        for key in self._scheduler_tasks.keys():
            if key.startswith(f"{phone}."):
                keys_to_remove.append(key)
    ...
    async def shutdown(self):
        """Cancel scheduled/running checkin tasks and unregister config callbacks."""
        tasks = list(self._tasks.values()) + list(self._scheduler_tasks.values())
        for site_tasks in self._site_tasks.values():
            tasks.extend(site_tasks.values())
        tasks.extend(self._pool.tasks)
    ...
    def _schedule_independent_site(self, account: TelegramAccount, site_name: str, config_to_use):
        ...
        self._start_scheduler(scheduler_key, scheduler)
    ...
    async def _run_single_site(self, ctx: RunContext, account: TelegramAccount, site_name: str):
        async with ClientsSession([account]) as clients:
            async for _, client in clients:
                clses = extract(get_cls("checkiner", names=[site_name]))
                if not clses:
                    logger.warning(f"无法找到站点 {site_name} 的签到器.")
                    return ctx.finish(RunStatus.FAIL, "无法找到签到器")
                cls = clses[0]
                config_to_use = account.checkiner_config or config.checkiner or CheckinerConfig()
    ...
    async def run_all(self, instant: bool = False):
        ...
        accounts = [a for a in self._telegram_accounts() if a.enabled and a.checkiner]
        tasks = [
    ...
    async def schedule_all(self):
        ...
        for a in self._telegram_accounts():
            if a.enabled and a.checkiner:
                scheduler = self.schedule_account(a)
                if scheduler:
                    self._start_scheduler(a.phone, scheduler)
