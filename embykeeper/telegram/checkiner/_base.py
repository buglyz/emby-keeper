...
class BaseBotCheckin(ABC):
    ...
    @property
    def retries(self):
        return self._retries if self._retries is not None else config.checkiner.retries

    @property
    def timeout(self):
        return self._timeout if self._timeout is not None else config.checkiner.timeout
