...
class OCRService:
    ...
    @staticmethod
    def _gif_frame_indices(frame_count: int, max_frames: int = 5):
        if frame_count <= 1:
            return [0]
        num_frames = min(max_frames, frame_count)
        return [i * (frame_count - 1) // (num_frames - 1) for i in range(num_frames)]
    ...
    async def _async_process_main(
        ...
        try:
            ...
            def process_gif(gif_data):
                ...
                frame_indices = OCRService._gif_frame_indices(frame_count)
