...
通常来说, 增加一个机器人的签到非常简单, 您需要在 `embykeeper/telegram/checkiner` 中增加一个文件 `dummy.py`:
...
更多签到器配置请参考 `embykeeper/telegram/checkiner/_base.py`.
...
│   │   │   └── ...
│   │   ├── registrar_main.py   # Telegram 定时抢注管理模块.
│   │   ├── registrar           # Telegram 定时抢注模块.
│   │   │   ├── __init__.py
│   │   │   ├── _base.py        # 定义基础抢注器.
│   │   │   └── ...
...
│   ├── utils.py                # 一系列工具函数和类.
│   └── var.py                  # 一系列全局变量.
├── embykeeperapi               # FastAPI 图形化管理平台包.
│   └─ ...
├── utils                       # 各测试工具.
│   └── ...
...
├── scripts
│   ├── docker-entrypoint.sh    # Docker 入口函数, 默认启动 FastAPI Web UI, 可通过 EK_MODE=cli 切换命令行模式.
├── tests
│   └── test_cli.py             # 基础测试文件.
├── tox.ini                     # 多版本测试文件
